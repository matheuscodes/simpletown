...
public class CitizenCache implements CacheInterface {
	...
	HashMap<String, Citizen> NPCs = null;
	...
	public boolean build() {
		...
		try {
			...
			count.next();
			size = count.getInt(1);
			NPCs = new HashMap<String, Citizen>(size);
			count.close();
			...
			while (all.next()) {
				...
				NPCs.put(all.getString(Citizen.ID_FIELD), new Citizen(all.getInt(Citizen.ID_FIELD), name, last_name, attributes));
				...
				NPCs.get(all.getString(Citizen.ID_FIELD)).setAspects(behavior, state_of_mind, bonding);
				// TODO optimize, avoid the "gets" and use variable
				if (all.getInt(Citizen.USER_ID_FIELD) != 0) {
					CacheServer.getUsers().getUser(all.getInt(Citizen.USER_ID_FIELD)).addCitizen(NPCs.get(all.getString(Citizen.ID_FIELD)));
				}
			}
			all.close();
